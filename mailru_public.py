...
class MailRuPublicClient:
    ...
    def _extract_date_from_filename(filename: str) -> Optional[datetime]:
        r"""Извлекает дату из имени файла.

        Поддерживает форматы:
          2024-01-15...        → ISO
          20130915_155734.jpg  → камера/Android (год+месяц+день, далее _ или что-то ещё)
          IMG_20210715.jpg     → iPhone (после префикса)
          15.01.2024...        → DD.MM.YYYY

        Использует (?<!\d) и (?!\d) вместо \b, чтобы ловить даты рядом с
        любыми НЕ-цифровыми символами, включая `_` (word character — \b не срабатывает).
        """
        patterns = [
            r'(?<!\d)(\d{4})-(\d{2})-(\d{2})(?!\d)',     # 2024-01-15
            r'(?<!\d)(\d{4})(\d{2})(\d{2})(?!\d)',       # 20240115 / 20130915_...
            r'(?<!\d)(\d{2})\.(\d{2})\.(\d{4})(?!\d)',   # 15.01.2024
        ]
